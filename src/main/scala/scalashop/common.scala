...
def boxBlurKernel(src: Img, x: Int, y: Int, radius: Int): RGBA =
  ...
  for ( boxX <- clamp(x - radius, 0, src.width - 1) to clamp(x + radius, 0, src.width - 1);
        ...
        alphaSum += alpha(neighbor)
        countNeighbors += 1
  }
  rgba(redSum/countNeighbors, greenSum/countNeighbors, blueSum/countNeighbors, alphaSum/countNeighbors)
